 ...
 */
public class QQView1 extends View {
    ...
    public QQView1(Context context) {
        super(context);
        mContext = context;
        init();
    }

    public QQView1(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }

    public QQView1(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        //自定义View的宽高比例
        mRatio = 450.f / 525.f;
        //初始化一些默认的参数 ViewUtils.dp2px(mContext, 5);
        mBackgroundCorner = 30;
    ...
    public void startAnim() {
        //一般使用AnimatorSet都是作用在ObjectAnimator上
        ...
        stepAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                percent = step / 9450f;
                Log.i("LOG", "step:" + step + "percent:" + percent);
                postInvalidate();
            }
        });
        stepAnimator.setDuration(2000);
        stepAnimator.setStartDelay(100);
        stepAnimator.start();
  /*      //圆环动画
   ...
   /*  percentAnimator.setDuration(2000);
        ...
        percentAnimator.setInterpolator(new AccelerateInterpolator());//刚开始慢之后快
        percentAnimator.start();*/
/*
     animatorSet.setDuration(4000);//设置动画时间,
       animatorSet.setStartDelay(100);//设置延迟时间
     animatorSet.setInterpolator(new LinearInterpolator());//设置差值器为匀速
      animatorSet.playSequentially(stepAnimator, percentAnimator);//逐个去取List中的动画对象，然后逐个播放
    animatorSet.playTogether(stepAnimator);//这个是两个同时进行,会发现view绘制的很快


        //让play里面的先执行,然后走with里面的
        // AnimatorSet.Builder builder = animatorSet.play(stepAnimator);
        // builder.with(percentAnimator);
         animatorSet.start();*/
    }
